    ...
    return (
        <div className="w-full grid grid-cols-12 gap-1 rounded-2xl border border-neutral-500 bg-opacityLight-5 px-3 py-3 items-center lg:px-6">
            ...
            <div className="text-right col-span-3">
                {available !== '-' && available !== '0' && <SecondaryButton onClick={handleClaim} className="w-fit text-neutral-200">Claim</SecondaryButton>}
                {(available === '-' || available === '0') && <SecondaryButton className="w-fit cursor-not-allowed bg-transparent border border-neutral-600 hover:bg-transparent text-neutral-500">Claim</SecondaryButton>}
            </div>
        </div>
    )
}