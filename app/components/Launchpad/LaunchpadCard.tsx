...
export default function LaunchpadCard({name, slug, isSoldOut, saleIsOpen, saleDate}: Projects) {
    ...
    return (
        <div className="relative">
            ...
            { isSoldOut && 
                <div className="absolute uppercase font-inter font-bold bg-beaige text-black top-2 left-2 py-1 px-2 text-[8px] md:text-xs md:top-4 md:left-4 lg:px-3 rounded-lg">Soldout</div>
            }
            { moment(saleDate).isAfter(moment(new Date()))  && 
                <div className="absolute flex items-center uppercase font-inter font-bold bg-green-blue text-black top-2 left-2 py-1 px-2 text-[8px] md:text-xs md:top-4 md:left-4 lg:px-3 rounded-lg">
