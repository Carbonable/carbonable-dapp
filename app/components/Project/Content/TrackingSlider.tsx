...
import type { Ndvi } from "~/types/dmrv";
  ...
  export default function TrackingSlider({data, setSelectedImageIndex, selectedDateIndex, setSelectedDateIndex}: {data: Ndvi[], setSelectedImageIndex: (index: number) => void, selectedDateIndex: number | undefined, setSelectedDateIndex: (index: number) => void}) {
      ...
      <div className="w-full overflow-hidden mx-auto min-h-[200px]">
        <Draggable bounds={{top: 0, left: -width, right: width, bottom: 0}} defaultPosition={{x: -width, y: 0}} {...dragHandlers} defaultClassName="w-fit mx-auto min-h-[200px]" onDrag={handleDrag}>
            <div className="box flex items-end justify-center cursor-grab">
                {data.map((item, index) => (
                    <div key={`date_${index}`} className={`w-[8px] rounded-t-full  border-t-[2px] border-x-[2px] border-greenish-700 ${selectedDateIndex && index <= selectedDateIndex ? "bg-greenish-500/80 border-greenish-800/80" : "bg-neutral-500/70 border-neutral-700/70"}`} style={{ height: `${Math.round(item.value * 100 * 0.9)}px`}}></div>
                ))}
            </div>
        </Draggable>
      </div>
