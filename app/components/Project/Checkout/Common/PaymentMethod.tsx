...
export default function PaymentMethod({ paymentMethod, setPaymentMethod }: { paymentMethod: PaymentMethodValues, setPaymentMethod: (paymentMethod: PaymentMethodValues) => void}) {
    const { activateStripePayment } = useNotifications();
            ...
            <div className="py-4 px-3 border-opacityLight-20">
                ...
                <div className="flex items-center mt-2">
                    <input 
                        ...
                        className="w-0 h-0 opacity-0 absolute"
                        disabled={!activateStripePayment}
                    />
                    <label 
                        htmlFor="credit_card" 
                        className={`flex w-full p-2 items-center justify-start rounded-lg border border-neutral-600 uppercase text-sm ${activateStripePayment ? "cursor-pointer" : "cursor-not-allowed brightness-50 hover:brightness-50"} ${paymentMethod === PaymentMethodValues.CREDIT_CARD ? "bg-neutral-100 text-neutral-900" : "bg-opacityLight-5 text-neutral-100 hover:brightness-110"}`}
                    >
                        <CreditCardIcon className="w-6 mr-2" />
                        Credit Card 
                        {activateStripePayment && <span className="text-xs text-neutral-300 normal-case ml-1"> ({STRIPE_FEES}% fees)</span>}
                        {!activateStripePayment && <span className="text-xs text-neutral-300 normal-case ml-1"> (SOON)</span>}
                    </label>
                </div>
            </div>
        </div>
