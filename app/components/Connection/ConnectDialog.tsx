                        ...
                        <div className="mt-6">
                            { connectors.map((wallet) => (
                                <div key={wallet.id + "_modal"}>
                                    ...
                                    {!wallet.available() && <a className="p-4 my-2 flex items-center justify-start cursor-pointer rounded-2xl bg-opacityLight-5 hover:bg-opacityLight-10 w-full" href={wallet.id === 'argentX' ? 'https://www.argent.xyz/argent-x/' : 'https://braavos.app/'} rel="noreferrer" target="_blank">
                                        <img className="w-5 h-5 mr-3" src={wallet.id === 'argentWebWallet' ? '/assets/images/common/argentX.svg' : `/assets/images/common/${wallet.id}.svg`} alt={`Connect with ${wallet.id}`} />
                                        <div className="uppercase font-inter mt-2">{wallet.id === 'argentWebWallet' ? 'Argent Web Wallet' : wallet.id}</div>
                                    </a>}
                                </div>
