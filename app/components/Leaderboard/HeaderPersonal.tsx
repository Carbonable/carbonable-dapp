...
export default function HeaderPersonal({ source, walletAddress }: { source: LeaderboardSource, walletAddress?: string }) {
    ...
    const [title, setTitle] = useState('Your points');
    ...
    useEffect(() => {
        if (source === LeaderboardSource.GLOBAL) {
    ...
    return (
        <div className="block border rounded-lg border-neutral-500 bg-opacityLight-10 p-4">
            ...
            <div className="mt-6">
                { !isConnected && source === LeaderboardSource.GLOBAL && <SecondaryButton onClick={handleConnect}>Connect wallet</SecondaryButton> }
                { isConnected && source === LeaderboardSource.GLOBAL && <SecondaryButton onClick={handleEarnMore}>Earn more points</SecondaryButton> }
            </div>
